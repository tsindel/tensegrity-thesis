function OF = minCsNorm(mode,s,mse,kse,le0,dle0,incidence_table,...
                        node_coords,j_bc,x_bc,maxCs_locs_na,dispOF)
    ...
    if strcmp(mode,'first')
        OF = objFcnCs(1,s,mse,kse,le0,dle0,incidence_table,node_coords,...
                      j_bc,x_bc) / maxCs_locs_na(1);
    elseif strcmp(mode,'second')
        OF = objFcnCs(2,s,mse,kse,le0,dle0,incidence_table,node_coords,...
                      j_bc,x_bc) / maxCs_locs_na(2);
    elseif strcmp(mode,'worst')
        Ba_norm = zeros(3*nn-nnz(x_bc),1);
        for i = 1:3*nn-nnz(x_bc) % compute sum of squares for the i-th mode
            Ba_norm(i) = objFcnCs(i,s,mse,kse,le0,dle0,incidence_table,...
                                  node_coords,j_bc,x_bc) / maxCs_locs_na(i);
